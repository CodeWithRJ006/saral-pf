...
// 1. Fix dummy TRACK THIS REQUEST button in timeline success state
...
// 2. Fix Two buttons -> One button for Fix Issue/Auto-Fix
content = content.replace(
  /<div className="flex flex-col sm:flex-row gap-3">[\s\S]*?<\/div>/,
...
// 3. Success block condition
content = content.replace(
  /\{\(scenario === "MISMATCH" && jdStatus === "success"\) \? \(/g,
  '{(scenario !== "CLEAN" && jdStatus === "success") ? ('
);

// 4. Update the Modal JSX to use dynamic variables
const oldModalJSX = `<div className="flex justify-between items-center mb-8 md:mb-12 border-b border-[#131215]/10 pb-4">
...
const newModalJSX = `<div className="flex justify-between items-center mb-8 md:mb-12 border-b border-[#131215]/10 pb-4">
...
// 5. Fix encoding string
content = content.replace(/A\xef\xbf\xbd\?sA13,50,000/g, '?13,50,000');
// In case the encoding is different, just do a generic regex:
...
console.log("Updated claims page.");
